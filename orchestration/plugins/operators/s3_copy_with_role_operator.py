...
class S3CopyWithRoleOperator(BaseOperator):
    ...
    def execute(self, context: Context) -> list[str]:
        ...
        if isinstance(
            self.source_bucket_key, str
        ) and not self.source_bucket_key.startswith("{{"):
            self.source_bucket_key = ast.literal_eval(self.source_bucket_key)
        if isinstance(
            self.dest_bucket_key, str
        ) and not self.dest_bucket_key.startswith("{{"):
            self.dest_bucket_key = ast.literal_eval(self.dest_bucket_key)
        logger.info(
            f"Parsing Source: s3://{self.source_bucket_name}/{self.source_bucket_key}"
        )
        logger.info(
            f"Parsing Destination: s3://{self.dest_bucket_name}/{self.dest_bucket_key}"
        )
        ...
        assumed_role = sts_client.assume_role(
            RoleArn=role_arn, RoleSessionName=f"s3_copy_{self.role_type}"
        )
        ...
        try:
            for source_bucket_key, dest_bucket_key in zip(
                self.source_bucket_key, self.dest_bucket_key
            ):
                copy_source = {
                ...
                logger.info(
                    f"Successfully copied object to s3://{self.dest_bucket_name}/{dest_bucket_key}"
                )
