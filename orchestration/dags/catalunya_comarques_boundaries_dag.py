...
def parse_extractor_response(**context):
    ...
    return {
        ...
        "dest_staging_multiple_keys": [
            single_key.replace("landing/", "staging/") for single_key in list_s3_keys
        ],
        "dest_mart_multiple_keys": [
            single_key.replace("landing/", "marts/") for single_key in list_s3_keys
        ],
    }
...
parse_response = PythonOperator(
    task_id="parse_response", python_callable=parse_extractor_response, dag=dag
)
