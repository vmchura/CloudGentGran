...
def create_service_type_payload(**context):
    ...
    logger.info(
        f"Created payload for service type catalog initializer: {json.dumps(payload, indent=2, default=str)}"
    )
...
def create_municipals_payload(**context):
    ...
    logger.info(
        f"Created payload for municipals catalog initializer: {json.dumps(payload, indent=2, default=str)}"
    )
    return json.dumps(payload)
...
def parse_service_type_response(**context) -> Dict[str, Any]:
    ...
    lambda_response = task_instance.xcom_pull(
        task_ids="invoke_service_type_initializer"
    )
    ...
    if lambda_response is None:
        logger.error(
            "Lambda response is None - this means the invoke_service_type_initializer task didn't run or failed"
        )
        raise AirflowException(
            "No Lambda response found - previous task may have failed"
        )
        ...
        try:
            ...
            raise AirflowException(
                f"Invalid JSON response from Lambda: {lambda_response}"
            )
        ...
        raise AirflowException(
            f"Unexpected Lambda response format: {type(lambda_response)}"
        )

    logger.info(
        f"Parsed response body: {json.dumps(response_body, indent=2, default=str)}"
    )

    # Check for Lambda execution errors (statusCode from the Lambda response)
    status_code = response_body.get("statusCode", 200)
    if status_code != 200:
        error_data = response_body.get("data", {})
        if error_data:
            error_msg = error_data.get("message", str(response_body))
        else:
            error_msg = str(response_body)

        logger.error(
            f"Service Type Catalog Initializer Lambda failed with status {status_code}: {error_msg}"
        )
        raise AirflowException(
            f"Service type catalog initialization failed: {error_msg}"
        )

    # Parse successful response data
    catalog_data = response_body.get("data", {})
    if not catalog_data:
        logger.error(f"No data field in response: {response_body}")
        raise AirflowException("Invalid Lambda response - missing 'data' field")
...
def parse_service_qualification_response(**context) -> Dict[str, Any]:
    ...
    # Get the Lambda response from the invoke task
    lambda_response = task_instance.xcom_pull(
        task_ids="invoke_service_qualification_initializer"
    )
    logger.info(
        f"Raw Service Qualification Lambda response received: {type(lambda_response)}"
    )

    # Parse Lambda response - handle different response formats
    if lambda_response is None:
        logger.error(
            "Lambda response is None - this means the invoke_service_qualification_initializer task didn't run or failed"
        )
        raise AirflowException(
            "No Lambda response found - previous task may have failed"
        )
    ...
    if status_code != 200:
        ...
        logger.error(
            f"Service Qualification Catalog Initializer Lambda failed with status {status_code}: {error_msg}"
        )
        raise AirflowException(
            f"Service qualification catalog initialization failed: {error_msg}"
        )
    ...
    # Enhanced logging for monitoring and debugging
    logger.info(
        f"✅ Service Qualification catalog initialization completed successfully:"
    )
    logger.info(f"   - Environment: {ENVIRONMENT}")
    logger.info(
        f"   - Function: {config['service_qualification_initializer_function']}"
    )
    logger.info(f"   - Table name: {catalog_data.get('table_name', 'N/A')}")
    logger.info(f"   - Records processed: {catalog_data.get('record_count', 'N/A')}")
    logger.info(f"   - S3 location: {catalog_data.get('s3_location', 'N/A')}")
...
def parse_municipals_response(**context) -> Dict[str, Any]:
    ...
    if lambda_response is None:
        logger.error(
            "Lambda response is None - this means the invoke_municipals_initializer task didn't run or failed"
        )
        raise AirflowException(
            "No Lambda response found - previous task may have failed"
        )
    ...
    logger.info(
        f"   - Extraction completed at: {municipals_data.get('extraction_completed_at', 'N/A')}"
    )
...
def validate_service_type_results(**context) -> str:
    ...
    catalog_data = task_instance.xcom_pull(
        task_ids="parse_service_type_response", key="service_type_metadata"
    )
    ...
    if not catalog_data:
        ...
        if not catalog_data:
            raise AirflowException(
                "No service type catalog metadata found - previous task may have failed"
            )
    ...
    if record_count <= 0:
        raise AirflowException(
            f"No records were processed (record_count: {record_count})"
        )
...
def validate_service_qualification_results(**context) -> str:
    ...
    # Get catalog metadata from previous task
    catalog_data = task_instance.xcom_pull(
        task_ids="parse_service_qualification_response",
        key="service_qualification_metadata",
    )
...
def validate_municipals_results(**context) -> str:
    ...
    municipals_data = task_instance.xcom_pull(
        task_ids="parse_municipals_response", key="municipals_metadata"
    )
    ...
    if not municipals_data:
        ...
        if not municipals_data:
            raise AirflowException(
                "No municipals metadata found - previous task may have failed"
            )
    ...
    if total_records <= 0:
        raise AirflowException(
            f"No records were extracted (total_records: {total_records})"
        )
    ...
    if total_records < min_expected_records:
        logger.warning(
            f"⚠️  Low record count detected for municipals: {total_records} < {min_expected_records}"
        )
        logger.warning(
            f"   This may indicate an issue with the data source or extraction logic"
        )
...
logger.info(
    f"   - Service Type Initializer function: {config['service_type_initializer_function']}"
)
logger.info(
    f"   - Service Qualification Initializer function: {config['service_qualification_initializer_function']}"
)
logger.info(
    f"   - Municipals Initializer function: {config['municipals_initializer_function']}"
)
...
(
    create_service_type_payload_task
    >> invoke_service_type_initializer
    >> parse_service_type_response
    >> validate_service_type_results
)

# Service Qualification pipeline
(
    create_service_qualification_payload_task
    >> invoke_service_qualification_initializer
    >> parse_service_qualification_response
    >> validate_service_qualification_results
)

# Municipals pipeline
(
    create_municipals_payload_task
    >> invoke_municipals_initializer
    >> parse_municipals_response
    >> validate_municipals_results
)
