...
def prepare_extractor_payload(**context) -> Dict[str, Any]:
    ...
    logger.info(
        f"Prepared extractor payload: {json.dumps(payload, indent=2, default=str)}"
    )
    return payload
...
def parse_extraction_response(**context) -> Dict[str, Any]:
    ...
    if lambda_response is None:
        ...
        raise AirflowException(
            "No Lambda response found - extractor task may have failed"
        )
...
def prepare_transformer_payload(**context) -> Dict[str, Any]:
    ...
    logger.info(
        f"Prepared transformer payload: {json.dumps(transformer_payload, indent=2)}"
    )
...
def validate_extraction_results(**context) -> str:
    ...
    if file_count <= 0:
        raise AirflowException(
            f"No files were created during extraction (file_count: {file_count})"
        )
    ...
    if total_records < min_expected_records:
        logger.warning(
            f"⚠️  Low record count detected: {total_records} < {min_expected_records}"
        )
        logger.warning(
...
def parse_transformation_response(**context) -> Dict[str, Any]:
    ...
    if isinstance(transformer_response, dict):
        ...
        try:
            ...
            raise AirflowException(
                f"Invalid JSON response from transformer: {transformer_response}"
            )
        ...
        if payload:
            try:
                ...
                raise AirflowException(
                    f"Invalid JSON payload from transformer: {payload}"
                )
        else:
            response_body = transformer_response
    else:
        logger.error(
            f"Unexpected transformer response type: {type(transformer_response)}"
        )
        raise AirflowException(
            f"Unexpected transformer response format: {type(transformer_response)}"
        )

    logger.info(
        f"Parsed transformer response: {json.dumps(response_body, indent=2, default=str)}"
    )
    ...
    logger.info(
        f"   - Output records: {transformation_data.get('clean_records', 'N/A')}"
    )
    logger.info(
        f"   - Files processed: {transformation_data.get('files_processed', 'N/A')}"
    )
    logger.info(
        f"   - Target location: {transformation_data.get('target_location', 'N/A')}"
    )
