...
def extract_source_prefix(**context):
    ...
    extractor_response = task_instance.xcom_pull(
        task_ids="population_municipal_greater_65_initializer"
    )
    extractor_response = (
        json.loads(extractor_response)
        if isinstance(extractor_response, str)
        else extractor_response
    )
...
def extract_transformer_target(**context):
    task_instance = context["task_instance"]
    transformer_response = task_instance.xcom_pull(
        task_ids="population_municipal_greater_65_transformer"
    )
    transformer_response = (
        json.loads(transformer_response)
        if isinstance(transformer_response, str)
        else transformer_response
    )
    ...
    return {"source_prefix": transformer_response.get("target_prefix")}


def validate_mart_completion(**context):
    task_instance = context["task_instance"]
    mart_response = task_instance.xcom_pull(
        task_ids="population_municipal_greater_65_mart"
    )
    mart_response = (
        json.loads(mart_response) if isinstance(mart_response, str) else mart_response
    )
    ...
    logger.info(
        f"Mart completed successfully. Target: {mart_response.get('target_prefix')}"
    )
...
prepare_mart_payload = PythonOperator(
    task_id="prepare_mart_payload", python_callable=extract_transformer_target, dag=dag
)
...
validate_mart = PythonOperator(
    task_id="validate_mart", python_callable=validate_mart_completion, dag=dag
)
