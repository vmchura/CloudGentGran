...
class TestLambdaFunction(unittest.TestCase):
    ...
    @patch.dict(
        os.environ, {"BUCKET_NAME": "test-bucket", "SEMANTIC_IDENTIFIER": "test-id"}
    )
        ...
        mock_responses = [
            {
                "link": {
                    "item": [
                        {
                            "label": "Cens de població i habitatges",
                            "href": "http://api/pop",
                        }
                    ]
                }
            ...
            {
                "link": {
                    "item": [{"label": "Per municipis", "href": "http://api/municipal"}]
                }
            },
    ...
    def test_fetch_href_base_url(self):
        href_base, error = fetch_href_base_url()
        assert error is None
        assert href_base is not None
        assert href_base == "https://api.idescat.cat/taules/v2/censph/540/19948/mun"
